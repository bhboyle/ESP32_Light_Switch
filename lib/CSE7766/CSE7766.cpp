...
// #if SENSOR_DEBUG
    // rdebugV("[SENSOR] CSE7766: _process: ");
    // for (byte i = 0; i < 24; i++)
    //     rdebugV("%02X ", _data[i]);
    // rdebugVln("\n");
// #endif
...
#if SENSOR_DEBUG
        debugE("[SENSOR] CSE7766: Checksum error");
#endif
        return;
    }
...
#if SENSOR_DEBUG
        debugE("[SENSOR] CSE7766: Chip not calibrated");
#endif
        return;
    }
    ...
    if ((_data[0] & 0xFC) > 0xF0)
    {
        ...
        if (0xF1 == _data[0] & 0xF1)
            debugE("[SENSOR] CSE7766: Abnormal coefficient storage area");
        if (0xF2 == _data[0] & 0xF2)
            debugE("[SENSOR] CSE7766: Power cycle exceeded range");
        if (0xF4 == _data[0] & 0xF4)
            debugE("[SENSOR] CSE7766: Current cycle exceeded range");
        if (0xF8 == _data[0] & 0xF8)
            debugE("[SENSOR] CSE7766: Voltage cycle exceeded range");
#endif
        return;
    }
